...
attack_model_bvm = bvmr(model_path="<?Add directory>/Imprints/watermark_removal_works/BVMR/demo_coco/checkpoints/demo_coco/net_baseline_200.pth",
                        device=device)


attackers = {
    'diff_attacker_60': DiffWMAttacker(att_pipe, noise_step=60,device=device),
...
def image_distortion(img1, img2, seed, attack, mask=None):
    if attack == 'rotation':
        img1 = transforms.RandomRotation((75, 75))(img1)
        img2 = transforms.RandomRotation((75, 75))(img2)
    ...
    if attack == 'reconstructionBVMR':
        img2.save('generated_imgs/plain.png') 
        img2 = attack_model.attack(img2)
        ...
        img2.save('generated_imgs/reconstructed.png')
...
def get_watermarking_pattern(pipe, args, device,image=None, shape=None,latent=None,dtype=None, alpha=None):
    set_random_seed(args.w_seed)
    # set_random_seed(10)  # test weak high freq watermark
    if shape is not None:
        gt_init = torch.randn(*shape, device=device)#.type(torch.complex32)
    ...
    if 'seed_ring' in args.w_pattern:  # spacial
        ...
        for i in range(args.w_up_radius, args.w_low_radius, -1):  
            ...
            gt_patch = torch.real(torch.fft.ifft2(torch.fft.ifftshift(combined_fft, dim=(-1, -2))))
